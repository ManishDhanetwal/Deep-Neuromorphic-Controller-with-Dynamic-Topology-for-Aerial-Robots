...
class NetEvo(object):
	def __init__(self,nInput,nOutput,initNode,slideWindow):
		...
		self.uNN_acc    = torch.zeros(slideWindow, requires_grad=True, dtype=torch.double)
		self.err_acc    = torch.zeros(slideWindow, requires_grad=True, dtype=torch.double)
		self.us_acc     = torch.zeros(slideWindow, requires_grad=True, dtype=torch.double)
		self.us_dot_acc = torch.zeros(slideWindow, requires_grad=True, dtype=torch.double)


	def calcBiasVar(self,forgetFactor,nWindow):
		# Mean Bias and Variance 
		sysBias                 = ((self.us_dot+self.us)-self.un)**2
	...
	def growNode(self):
		...
		W_add     = 0.01*torch.rand(1, nInputAdjusted,dtype=torch.double)
		W_out_add = 0.01*torch.rand(nOutput, 1,dtype=torch.double)
	...
	def controlUpdate(self):
		...
		Xe = np.array([self.err,self.lasterr,self.last2err])
	...
	def optimize(self,lr,alpha,slideWindow):
		# Optimize the neural network with adaptive learning rate

		self.uNN_acc    = torch.cat((self.uNN_acc,self.uNN),0)
		self.err_acc    = torch.cat((self.err_acc,torch.tensor([self.err],dtype=torch.double)),0)
		self.us_acc     = torch.cat((self.us_acc,torch.tensor([self.us],dtype=torch.double)),0)
		self.us_dot_acc = torch.cat((self.us_dot_acc,torch.tensor([self.us_dot],dtype=torch.double)),0)

		self.uNN_acc    = self.uNN_acc[-1*(slideWindow):]
		self.err_acc    = self.err_acc[-1*(slideWindow):]
		self.us_acc     = self.us_acc[-1*(slideWindow):]
		self.us_dot_acc = self.us_dot_acc[-1*(slideWindow):]
			#pdb.set_trace()
		self.cnt+=1

		if self.cnt % slideWindow == 0:
			self.cnt = 0
			mUNN = self.uNN_acc.mean()
			mErr = self.err_acc.mean()
			mUs  = self.us_acc.mean()
			mUsDot = self.us_dot_acc.mean()

			#mUNN.backward(torch.ones_like(mUNN))				# Backward the NN output
			mUNN.backward()
			#self.g = lr * (self.us_dot+self.us)
			self.g = lr * (mUsDot+mUs)		# Adaptive learning rate and SMC derivative update law

			for i in range(len(self.netStruct)):
				#pdb.set_trace()
				W = self.netStruct[i].linear.weight.data + self.g *self.netStruct[i].linear.weight.grad + alpha*self.netStruct[i].linear.weight.data
				self.netStruct[i].linear.weight.data  = W.clone().detach().requires_grad_(True)

				if self.netStruct[i].linear.bias is not None:
					b = self.netStruct[i].linear.bias.data + self.g*self.netStruct[i].linear.bias.grad + alpha*self.netStruct[i].linear.bias.data
...
# netEv = NetEvo(N_INPUT,N_OUTPUT,INIT_NODE,2)
...
# 	netEv.optimize(LEARN_RATE,WEIGHT_DECAY,2)
# 	netEv.growNode()
