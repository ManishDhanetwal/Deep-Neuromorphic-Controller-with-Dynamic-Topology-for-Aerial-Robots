...
# Ignite the Evolving NN Controller
    ...
    Wx0=torch.tensor([[-0.0324, -0.0259, -0.0353,  0.0019]], dtype=torch.float64,requires_grad = True)
    Wx1=torch.tensor([[-0.6233]], dtype=torch.float64,requires_grad = True)
    # Wx2=torch.tensor([[0.3805, 0.5764, 0.6651]], dtype=torch.float64,requires_grad = True)

    Wy0=torch.tensor([[-0.0055, -0.0201, -0.0097, -0.0035]], dtype=torch.float64,requires_grad = True)
    Wy1=torch.tensor([[-0.5789]], dtype=torch.float64,requires_grad = True)
