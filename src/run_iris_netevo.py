...
# INITIAL SETUP
    ...
    PID_GAIN_Z    = [0.013,0.0005,0.2]      # PID Gain Parameter [KP,KI,KD] axis-Z
    #PID_GAIN_Z    = [0.013,0.0,0.2]         # PID Gain Parameter [KP,KI,KD] axis-Z
    SIM_TIME      = 80                     # Simulation time duration (s)
...
# ROS Main loop::
    ...
    while not rospy.is_shutdown():
        ...
        # elif runtime > 20 and runtime <=30:
        ...
        if runtime <= 20:
            xref = 0.0
            yref = 0.0
            zref = trajectory.linear(1.0,dt,20)
        ...
        # SMC + NN controller
        vx  = Xcon.controlUpdate()      # Velocity X
        vy  = Ycon.controlUpdate()      # Velocity Y
        thz = Zcon.controlUpdate()      # Additional Thrust Z
        ...
        # Append Data Logger
        logData.appendStateData(runtime,execTime,xref,yref,zref,xpos,ypos,zpos,phi_ref,theta_ref,thrust_ref)
        ...
        for i in range(len(Zcon.netStruct)):
            print Zcon.netStruct[i].linear.weight.data
        ...
        if runtime > SIM_TIME:
            ...
            logData.saveAll()
            xconLog.saveControl('X')
            yconLog.saveControl('Y')
            zconLog.saveControl('Z')
