...
# INITIALIZE ADAPTIVE NETWORK PARAMETERS:
    ...
    INIT_NODE_X     = 3                   # Number of node(s) for initial structure
    INIT_LAYER_X    = 3                   # Number of initial layer (minimum 2, for 1 hidden and 1 output)
    INIT_NODE_Y     = 2                   # Number of node(s) for initial structure
    ...
    LEARNING_RATE_Z = 0.25             # Network optimization learning rate
...
# INITIALIZE SYSTEM AND SIMULATION PARAMETERS:
    ...
    SIM_TIME      = 400                     # Simulation time duration (s)
#--------------------------------------------------------------------------------
...
# Ignite the Evolving NN Controller
    ...
    Wx0=torch.tensor([[ 0.3072,  0.3056,  0.3047, -0.0332],
        [ 0.3088,  0.3041,  0.3027, -0.0359],
        [ 0.3046,  0.3025,  0.3037, -0.0348]], dtype=torch.float64,requires_grad = True)
    Wx1=torch.tensor([[0.3354, 0.3356, 0.3354],
        [0.3978, 0.3976, 0.3963],
        [0.4259, 0.4254, 0.4238]], dtype=torch.float64,requires_grad = True)
    Wx2=torch.tensor([[0.3805, 0.5764, 0.6651]], dtype=torch.float64,requires_grad = True)

    Wy0=torch.tensor([[ 0.0599,  0.0588,  0.0591, -0.0056],
        [-0.1304, -0.1269, -0.1258,  0.0064]], dtype=torch.float64,requires_grad = True)
    Wy1=torch.tensor([[ 0.3893, -0.8508]], dtype=torch.float64,requires_grad = True)

    Wz0=torch.tensor([[0.2172, 0.2112, 0.2059, 0.0199]], dtype=torch.float64,requires_grad = True)
    Wz1=torch.tensor([[0.3713]], dtype=torch.float64,requires_grad = True)
    ...
    while not rospy.is_shutdown():
       ...
       # Xcon.optimize(LEARNING_RATE_X,WEIGHT_DECAY)
        ...
        if runtime > SIM_TIME:
            ...
            saveParameters("circle",Xcon.netStruct,Ycon.netStruct,Zcon.netStruct)
