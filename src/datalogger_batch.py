...
class dataLogger(object):
        ...
        def __init__(self,logType,runType):
            self.ticktime = []
            self.ticks = []
            self.exeTime = []
            self.x_ref = []
            ...
            self.path    = "/home/c8/codes/evonn_ros_mocap_version-05/Batch_simulation_multi_task/"

            if logType == 'flight':
                self.logFile = open(self.path+runType+"_FlightStateLog"+self.timestr+".txt", "w+")
            else:
                self.logFile = open(self.path+runType+"_"+logType+"-ControlLog"+self.timestr+".txt", "w+")
        ...
        def saveStateData(self,t,ext,xr,yr,zr,xa,ya,za,phir,thetar,thrustr):
            self.logFile.writelines("%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%f\n"%\
                (t,ext,xr,yr,zr,xa,ya,za,phir,thetar,thrustr))
        ...
        def saveControlData(self,t,usmc,unn,layer,node):
            self.logFile.writelines("%f;%f;%f;%d;%d\n"%\
                (t,usmc,unn,layer,node))
...
def saveParameters(runType,netX,netY,netZ):
    path = "/home/c8/codes/evonn_ros_mocap_version-05/Batch_simulation_multi_task/"
    logW = open(path+runType+"_Weights"+time.strftime("%Y%m%d-%H%M%S")+".txt", "w+")
    
    logW.writelines("X-net: "+str(netX)+"\n")
    for i in range(len(netX)):
        logW.writelines("weight of layer-%d = "%i + str(netX[i].linear.weight.data)+"\n")
    
    logW.writelines("\n")
    logW.writelines("Y-net: "+str(netY)+"\n")
    for i in range(len(netY)):
        logW.writelines("weight of layer-%d = "%i + str(netY[i].linear.weight.data)+"\n")
    
    logW.writelines("\n")
    logW.writelines("Z-net: "+str(netZ)+"\n")
    for i in range(len(netZ)):
        logW.writelines("weight of layer-%d = "%i + str(netZ[i].linear.weight.data)+"\n")
