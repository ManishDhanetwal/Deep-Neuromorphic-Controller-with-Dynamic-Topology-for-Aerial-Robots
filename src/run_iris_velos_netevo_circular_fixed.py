...
# INITIALIZE ADAPTIVE NETWORK PARAMETERS:
    ...
    LEARNING_RATE_X = 0.025             # Network optimization learning rate 0.00085
    LEARNING_RATE_Y = 0.025             # Network optimization learning rate 0.00085
    LEARNING_RATE_Z = 0.15             # Network optimization learning rate
...
# INITIALIZE SYSTEM AND SIMULATION PARAMETERS:
    ...
    SIM_TIME      = 300                     # Simulation time duration (s)
#--------------------------------------------------------------------------------
...
# Ignite the Evolving NN Controller
    ...
    # Init weight 1st run
    # Wx0=torch.tensor([[ 0.7658,  0.7642,  0.7660, -0.1051]], dtype=torch.float64,requires_grad = True)
    # Wx1=torch.tensor([[1.3815]], dtype=torch.float64)

    # Wy0=torch.tensor([[ 0.7665,  0.7633,  0.7626, -0.0517]], dtype=torch.float64,requires_grad = True)
    # Wy1=torch.tensor([[1.3603]], dtype=torch.float64)

    # Wz0=torch.tensor([[0.1849, 0.1906, 0.1868, 0.0697]], dtype=torch.float64,requires_grad = True)
    # Wz1=torch.tensor([[0.3313]], dtype=torch.float64,requires_grad = True)

    # Init weight 2nd run (continuously learning)
    Wx0=torch.tensor([[ 0.7658,  0.7642,  0.7660, -0.1051]], dtype=torch.float64,requires_grad = True)
    Wx1=torch.tensor([[1.3815]], dtype=torch.float64)
...
# ROS Main loop::
    ...
    while not rospy.is_shutdown():
        ...
        #     xref = trajectory.linear(1.0,dt,20)
        #     yref = 0.0
        #     zref = 1.0
        ...
        # if runtime > 15 :
        #     xref,yref = trajectory.circle(1.0,dt,20.0)
        #     zref = 1.0
            # if runtime == 200:
            #     trajectory.reset()
            #     xs=xpos
        ...
        #     xref = xs+trajectory.linear(1.0,dt,10.0)
        #     yref = ys
        #     zref = 1.0
        ...
        # if runtime > 210 and runtime <=400:
        #     xr,yr = trajectory.circle(1.0,dt,20.0)
        #     xref = xs+xr
        #     yref = ys+yr
        #     zref = 1.0
        ...
        # if runtime > 400 and runtime <=410:
        #     xref = xs
        #     yref = ys+trajectory.linear(1.0,dt,10.0)
        #     zref = 1.0
        ...
        # # helix
        # if runtime > 45:
        #     xref,yref, zref = trajectory.helix(1.0,1.0,dt,20,155)


        # Square
        if runtime > 15:
            xref,yref = trajectory.square(1.5,dt,10.0)
            zref = 1.0
        ...
        for i in range(Xcon.nLayer):
            print 'weight X layer-%d = '%i,Xcon.netStruct[i].linear.weight.data
            print 'weight Y layer-%d = '%i,Ycon.netStruct[i].linear.weight.data
            print 'weight Z layer-%d = '%i,Zcon.netStruct[i].linear.weight.data
        ...
        if runtime > SIM_TIME:
            ...
            saveParameters(Xcon.netStruct,Ycon.netStruct,Zcon.netStruct)
