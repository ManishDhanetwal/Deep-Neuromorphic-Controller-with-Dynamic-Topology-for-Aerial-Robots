...
class NetEvo(object):
	...
	def adjustDepth(self,ETA,DELTA,N_WINDOW,EVAL_WINDOW):
		...
		if self.cnt % (EVAL_WINDOW*N_WINDOW) == 0 and self.cnt!=0:		
			...
			if self.gradBias > 0 and abs(self.gradBias - self.minGradBias) < 1.25 * right and self.nNodes>=2:
				#pdb.set_trace()
				self.addhiddenlayer()
				self.nLayer =len(self.netStruct)
				self.minGradBias = self.gradBias
	...
	def optimize(self,lr,alpha):
		...
		self.uNN.backward()							# Backward the NN output
		...
		for i in range(nLayers):
			...
			if i==0:
				#pdb.set_trace()
				print ('weight       = ', self.netStruct[0].linear.weight.data)
