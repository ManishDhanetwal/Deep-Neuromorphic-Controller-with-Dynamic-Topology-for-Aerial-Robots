...
# INITIALIZE ADAPTIVE NETWORK PARAMETERS:
    ...
    LEARNING_RATE_Z = 0.075                # Network optimization learning rate 0.06, 0.075,0.1

    WEIGHT_DECAY  = 0.00015            # Regularization weight decay factor 0.000125, 0.00005
...
# INITIALIZE SYSTEM AND SIMULATION PARAMETERS:
    ...
    PID_GAIN_X    = [1.25,0.0,0.0]         # PID Gain Parameter [KP,KI,KD] axis-X 0.45,0.9
    PID_GAIN_Y    = [2.5,0.0,0.0]         # PID Gain Parameter [KP,KI,KD] axis-Y 0.45,0.9,1.8
    ...
    PID_GAIN_Z    = [1.25,0.0,0.0]         # PID Gain Parameter [KP,KI,KD] axis-Z
    SIM_TIME      = 355                     # Simulation time duration (s)
#--------------------------------------------------------------------------------
...
# Ignite the Evolving NN Controller
    ...
    #Init Weight
    Wz0=torch.tensor([[ 0.2143,  0.2148,  0.2133, -0.0669]], dtype=torch.float64,requires_grad = True)
    Wz1=torch.tensor([[1.0455]], dtype=torch.float64,requires_grad = True)
...
# ROS Main loop::
    ...
    while not rospy.is_shutdown():
        ...
        if runtime > 30 and runtime <=185:
            xref = -0.9
            yref = 1.0
            zref = 1.0 + trajectory.sinusoid(0.4,dt,20)
        ...
        # Hold under the  Ceiling for 6 times
        if runtime > 210 and runtime <=330:
            xref = -0.9
            yref = -1.67
            zref = 1.0 + trajectory.sinusoid(0.4,dt,20)
            # zref = 1.2
        ...
        # Go to Z = 1.0
        if runtime > 330 and runtime <=335:
            xref = -0.9
            yref = -1.67
            zref = zr[j]
            j   += 1
            ...
            if runtime == 335:
                ...
                ys = uav.local_pos.y
                j  = 0
                yr = np.linspace(ys,1.0,num=(10.0/dt))
        
        # Go to Y= 1
        if runtime > 335 and runtime <=345:
            xref = -0.9
            yref = yr[j]
            zref = 1.0
            j   += 1
